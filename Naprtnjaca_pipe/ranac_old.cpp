...
{
	/* privremeno */
	static int velicine[]={6,4,1};
	static int cijene[]={8,5,1};
	static int kapacitet = 29;
	static int ogranicenja[3];	
	static int rjesenje[3];
	/* kraj */
	...
	rval=kreirajListu(h0,velicine,3);
	rval=kreirajListu(h0+1,ogranicenja,3);
	rval=PL_put_integer(h0+2,kapacitet);
	rval=kreirajListu(h0+3,cijene,3);
	rval=kreirajImenaVarijabli(h0+4,3);
	
	varijable_functor = PL_new_functor(PL_new_atom("varijable"),3);
	v0 = PL_new_term_refs(3);
	for(i=0; i<3; i++)
